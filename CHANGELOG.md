...
- Coverage floor is now **two-tier**: 85% cross-platform baseline (still in `pyproject.toml`) plus **87% enforced on the integration-tests CI job** which runs the Postgres, MySQL, and SQL Server service containers. The integration-tests job uses `--cov-fail-under=87` to fail any PR that drops the canonical measured coverage below 87%. Cross-platform tests on macOS/Windows continue to enforce 85% locally.
