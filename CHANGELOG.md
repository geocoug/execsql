...
- `BREAKPOINT` REPL now accepts multi-line SQL: any non-dot input that isn't a bare variable name opens a buffer with the continuation prompt ` ...        >`, accumulating lines until one ends with `;`. Use `.cancel` (or Ctrl-C / EOF) to discard a partial buffer. Dot-commands still work mid-buffer. A bare SQL keyword (`select`, `update`, `delete`, `with`, `create`, `drop`, etc.) on its own line is also treated as the start of multi-line SQL instead of an undefined-variable lookup.
